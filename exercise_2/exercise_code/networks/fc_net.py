...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        # and biases using the keys 'W2' and 'b   2'.                                 #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        scores_max, cache_max = affine_relu_forward(X, W1, b1)
        scores, cache_affine = affine_forward(scores_max, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        loss_softmax, d_softmax = softmax_loss(scores, y)
        loss_reg_1, _ = l2_loss(W1, np.zeros_like(W1))
        loss_reg_2, _ = l2_loss(W2, np.zeros_like(W2))

        loss = 0.5 * reg * (loss_reg_1**2 + loss_reg_2**2) + loss_softmax

        d_relu, grads['W2'], grads['b2'] = affine_backward(d_softmax, cache_affine)
        grads['W2'] += reg * W2

        d_affine_relu, grads['W1'], grads['b1'] = affine_relu_backward(d_relu, cache_max)
        grads['W1'] += reg * W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        ############################################################################
        total_dims = np.hstack([input_dim, hidden_dims, num_classes])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, L):
            input, cache['cache'+str(i)] = affine_relu_forward(input, self.params['W'+str(i)], self.params['b'+str(i)])

        scores, cache_affine = affine_forward(input, self.params['W'+str(L)], self.params['b'+str(L)])
        ...
        loss_reg = 0
        for i in range(L):
            l2_norm['W'+str(i+1)], _ = l2_loss(self.params['W'+str(i+1)], np.zeros_like(self.params['W'+str(i+1)]))
            loss_reg += reg * l2_norm['W'+str(i+1)]**2
        loss = loss_softmax + loss_reg

        d_out, grads['W'+str(L)], grads['b'+str(L)] = affine_backward(d_softmax, cache_affine)
        for i in reversed(range(1, L)):
            d_out, grads['W'+str(i)], grads['b'+str(i)] = affine_relu_backward(d_out, cache['cache'+str(i)])

        for i in range(L):
            grads['W'+str(i+1)] += reg * grads['W'+str(i+1)]
