...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # array of shape (N, C).                                               #         
        ########################################################################
        scores_input = X.dot(W1) + b1
        scores_max = np.maximum(scores_input, 0)
        scores = scores_max.dot(W2) + b2
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        loss = 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2)) + np.sum(-np.log(prob[range(N), y])) / N
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        deriv_score = prob
        deriv_score[range(np.shape(X)[0]), y] -= 1
        grads['W2'] = scores_max.T.dot(deriv_score) / np.shape(X)[0] + reg * W2
        grads['b2'] = np.sum(deriv_score, axis=0) / np.shape(X)[0]

        deriv_score_input = (deriv_score / np.shape(X)[0]).dot(W2.T)
        deriv_score_input[scores_input <= 0] = 0
        grads['W1'] = X.T.dot(deriv_score_input) + reg * W1
        grads['b1'] = np.sum(deriv_score_input, axis=0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            ####################################################################
            batch_size_indices = np.random.choice(num_train, batch_size, replace=True)
            X_batch = X[batch_size_indices]
            y_batch = y[batch_size_indices]
            ####################################################################
            #                             END OF YOUR CODE                     #
            ####################################################################
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    results = {}
    best_val = -1
    learning_rates = [1.2e-3, 1.4e-3, 1.6e-3, 1.8e-3]
    ...
    for lr in learning_rates:
        for rs in regularization_strengths:
            ...
            train_history = net.train(X_train, y_train, X_val, y_val,
